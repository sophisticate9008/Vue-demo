...
export default class MainContent {
  ...
  private _renderFindBox(): HTMLElement {
    ...
    btnEl.innerHTML = '查找<i></i>';
    btnEl.addEventListener('click', () => {
      let _type: Type = this.props.type === 'find' ? 'replace' : 'find';
      this.props.type = _type;
      btnEl.classList.toggle('active', this.props.type === 'replace');
    });
    el.btnEl = btnEl;
  ...
  private _renderReplaceBox(): HTMLElement {
    const el: any = document.createElement('DIV');
    el.classList.add(`${MainContent.NAMESPACE_CLASS}--replace`);
  ...
  private _render(parentContainer: any): HTMLElement {
    ...
    parentContainer.replaceBox = this._renderReplaceBox();

    parentContainer.append(
      parentContainer.moveBox,
      parentContainer.closeBox,
      parentContainer.findBox,
      parentContainer.replaceBox
    );
